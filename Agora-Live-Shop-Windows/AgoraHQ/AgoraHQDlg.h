...
#include "agora_video_source.h"//advertise
...
public:
	...
	LRESULT onRequestToken(WPARAM wParam,LPARAM lParam);
...
//	AG_INVITE_REMOTEAUDIENCE m_agInviteRemoteAudience;
	...
	void GetVideoParameter(CString param, int& w, int& h, int& fps, int bitrate);
...
private:
	...
	//obs set video ouput parameter
	int obs_output_x = 640;
	int obs_output_y = 360;
	int obs_videoBitrate = 400;
	std::string obs_video_fps = "15";
	int obs_fps = 15;
	VIDEO_PROFILE_TYPE obs_video_profile_type = VIDEO_PROFILE_PORTRAIT_360P;
