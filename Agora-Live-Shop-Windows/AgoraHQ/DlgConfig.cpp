...
void CDlgConfig::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_EDIT_AdvertisMediaUID, m_edtAdvertiseMediaUID);
	DDX_Control(pDX, IDC_COMBO_AdvertiseVideoSolution, m_cmbAdvertiseVideoProfile);
	DDX_Control(pDX, IDC_COMBO_AdvertiseCAMERA, m_cmbAdvertiseCamera);
}
...
BEGIN_MESSAGE_MAP(CDlgConfig, CDialogEx)
	...
	ON_CBN_SELCHANGE(IDC_COMBO_CAMERA, &CDlgConfig::OnSelchangeComboCamera)
	ON_CBN_SELCHANGE(IDC_COMBO_AdvertiseCAMERA, &CDlgConfig::OnSelchangeComboAdvertisecamera)
	ON_WM_TIMER()
END_MESSAGE_MAP()
...
CString CDlgConfig::getVideoParam()
{
	CString strVideoParam;
	m_comSolutionIndex.GetWindowText(strVideoParam);
...
void CDlgConfig::OnBnClickedButtonConfigSave()
{
	...
	int nAdSel = m_cmbAdvertiseCamera.GetCurSel();

	IRtcEngine* pRtcEngine = CAgoraObject::GetEngine();
	if (pRtcEngine && CB_ERR != nAdSel && nCurSel != CB_ERR){
		if (nAdSel == nCurSel){
			AfxMessageBox(_T("The camera advertisement used shouldn't be the same with broadcaster"));
			return;
		}
	}
	...
	m_edtAdvertiseMediaUID.GetWindowTextW(strParam);
	gHQConfig.setAdvertiseUid(cs2s(strParam));

	int nAdVideoIndexCurSel = m_cmbAdvertiseVideoProfile.GetCurSel();
	gHQConfig.setAdvertiseVideoProfile(int2str(nAdVideoIndexCurSel));
	if (pAgoraHQDlg)
		pAgoraHQDlg->SetVideoSourceVideoProfile(nAdVideoIndexCurSel);
	...
	if (pRtcEngine && CB_ERR != nAdSel ){
		if (CB_ERR == nCurSel)
			CameraManager.Create(pRtcEngine);
		CString strDeviceName; CString strDeviceId;
		CameraManager.GetDevice(nAdSel, strDeviceName, strDeviceId);
		gHQConfig.setAdvertiseCameraDeviceName (cs2s(strDeviceName));
...
void CDlgConfig::initData()
{
	...
	for (int nIndex = 0; nIndex < 32; nIndex++){
		...
		m_cmbAdvertiseVideoProfile.AddString(m_szProfileDes[nIndex]);
		m_cmbAdvertiseVideoProfile.SetItemData(nIndex, (DWORD_PTR)m_nProfileValue[nIndex]);
	}
}
...
void CDlgConfig::initCtrl()
{
	...
	std::string strAdMediaUid = gHQConfig.getAdvertiseUid();
	strAdMediaUid = "2";
	m_edtAdvertiseMediaUID.SetWindowTextW(s2cs(strAdMediaUid));
	m_edtAdvertiseMediaUID.EnableWindow(FALSE);
	...
	std::string strAdVideoProfile = gHQConfig.getVideoSolutinIndex();
	if ("" == strAdVideoProfile)
		strVideoIndex = "15";
	m_cmbAdvertiseVideoProfile.SetCurSel(str2int(strAdVideoProfile));

	if (pAgoraHQDlg)
		pAgoraHQDlg->SetVideoSourceVideoProfile(m_cmbAdvertiseVideoProfile.GetCurSel());

	std::string strCameraID = gHQConfig.getCameraDeviceID();
	CString strDefCameraId = s2cs(strCameraID);
	CString str = _T("initCtrl broadcast: ") + strDefCameraId;
	
	std::string strAdCameraID = gHQConfig.getAdvertiseCameraDeviceID();
	CString strAdDefCameraId = s2cs(strAdCameraID);

	int nDefSel = CB_ERR;
	int nDefAdSel = CB_ERR;
	...
	if (pRtcEngine){
		...
		if (CameraManager.Create(pRtcEngine)){
			...
			for (int nDeviceId = 0; nDeviceId < nCameraCount; nDeviceId++){
				...
				if (nCameraCount > 1){
					m_cmbAdvertiseCamera.AddString(strDeviceName);
					if (strAdDefCameraId == strDeviceId){
						nDefAdSel = nDeviceId;
					}
				}
			}
	...
	if (nDefSel != CB_ERR){
		...
		CameraManager.SetCurDevice(s2cs(gbk2utf8(cs2s(strDefCameraId))));
	}
	

	SetTimer(idEvent, 1000, NULL);
	if (nDefAdSel != CB_ERR){
		m_cmbAdvertiseCamera.SetCurSel(nDefAdSel);
		if (pAgoraHQDlg)
			pAgoraHQDlg->SetVideoSourceCamera(gbk2utf8(cs2s(strAdDefCameraId)).c_str());
	}
...
void CDlgConfig::EnableAdvertiseCtrl(BOOL bEnable)
{
	m_edtAdvertiseMediaUID.EnableWindow(bEnable);
	m_cmbAdvertiseCamera.EnableWindow(bEnable);
	m_cmbAdvertiseVideoProfile.EnableWindow(bEnable);
}
...
void CDlgConfig::OnSelchangeComboCamera()
{
...
void CDlgConfig::OnSelchangeComboAdvertisecamera()
{
	
}

int CDlgConfig::getVideoSourceProfile()
{
	int nVideoProfileIndex = m_cmbAdvertiseVideoProfile.GetCurSel();
	if (nVideoProfileIndex == CB_ERR){
		return VIDEO_PROFILE_TYPE::VIDEO_PROFILE_DEFAULT;
	}
	return m_cmbAdvertiseVideoProfile.GetItemData(nVideoProfileIndex);
}
