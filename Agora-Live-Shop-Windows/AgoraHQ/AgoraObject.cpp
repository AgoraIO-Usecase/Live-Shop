...
BOOL CAgoraObject::MuteRemoteAudio(uid_t uid, BOOL bMuted)
{
	ASSERT(m_lpAgoraEngine != NULL);
...
BOOL CAgoraObject::MuteRemoteVideo(uid_t uid, BOOL bMuted)
{
	ASSERT(m_lpAgoraEngine != NULL);
...
CStringA CAgoraObject::getDynamicMediaChannelKey(CString channelname)
{
	if (_T("") != channelname){
		...
		auto  expiredTs = 0;
		std::string meidaChannelKey = agora::tools::DynamicKey5::generateMediaChannelKey(appID, appCertificate, channelName, unixTs, randomInt, uid, expiredTs);
