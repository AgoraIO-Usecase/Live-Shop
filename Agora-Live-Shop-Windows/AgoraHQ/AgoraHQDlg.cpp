...
void CAgoraHQDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_STATIC_ADVERTISE, m_ctlAdvertise);
...
BEGIN_MESSAGE_MAP(CAgoraHQDlg, CDialogEx)
	...
	ON_MESSAGE(WM_MSGID(EID_REQUEST_TOKEN), onRequestToken)
...
LRESULT CAgoraHQDlg::onJoinChannelSuccess(WPARAM wParam, LPARAM lParam)
{
	...
	//m_btnHQConfig.EnableWindow(FALSE);
	return 0;
}
...
LRESULT CAgoraHQDlg::onLeaveChannel(WPARAM wParam, LPARAM lParam)
{
	...
	m_btnHQConfig.EnableWindow(TRUE);

	m_ctlShowPic.Invalidate();
	m_ctlAdvertise.Invalidate();
	return TRUE;
}

LRESULT CAgoraHQDlg::onRequestToken(WPARAM wParam, LPARAM lParam)
{
	if (m_lpRtcEngine){
		CString strChannelName = m_lpAgoraObject->GetChanelName();
		CStringA strNewMediaChannelKey = m_lpAgoraObject->getDynamicMediaChannelKey(strChannelName);
...
void CAgoraHQDlg::GetVideoParameter(CString param, int& w, int& h, int& fps, int bitrate)
{
	param.TrimLeft();
	param.TrimRight();
	...
	pos = param.Find(_T(" "));
	if (pos >= 0){
		CString str = param.Mid(0, pos);
		param = param.Mid(pos + 1);
		pos = str.Find(_T("x"));
		w = _ttoi(str.Mid(0, pos).GetBuffer());
		h = _ttoi(str.Mid(pos + 1).GetBuffer());

		pos = param.Find(_T("fps"));
		if (pos >= 0){
			str = param.Mid(0, pos);
			param = param.Mid(pos + 4);
			fps = _ttoi(str.GetBuffer());

			pos = param.Find(_T("kbps"));
			if (pos >= 0){
				str = param.Mid(0, pos);
				bitrate = _ttoi(str.GetBuffer());
			}
		}
	}
...
void CAgoraHQDlg::JoinChannel_OBS()
{
	...
	GetVideoParameter(m_pDlgConfig->getVideoParam(), obs_output_x, obs_output_y, obs_fps, obs_videoBitrate);

	CAgoraObject::GetAgoraObject()->SetAudioProfile(44100, 2, 2*1024);
	int scale_cx = obs_output_x;
	int scale_cy = obs_output_y;
	config_set_default_uint(basicConfig, "Video", "OutputCX", scale_cx);
	config_set_default_uint(basicConfig, "Video", "OutputCY", scale_cy);
	...
	agoraOutputHandler->SetEncoderCallback(CAgoraHQDlg::obsVideoCallback, NULL);//CAgoraHQDlg::obsAudioCallback);
...
void CAgoraHQDlg::InitAgoraVideoSource()
{
	...
	m_videoSourceSink.reset(createVideoSource());//advertise
	m_videoSourceSink->initialize(&m_videoSourceEventHandler, m_ctlAdvertise.GetSafeHwnd(), m_strAppId.c_str(), token.c_str());
	m_lpAgoraObject->MuteRemoteVideo(m_advertiseUid);
}

int CAgoraHQDlg::SetVideoSourceCamera(const char* cameraId)
{
	return m_videoSourceSink->setCurrentCamera(cameraId);
}

int CAgoraHQDlg::SetVideoSourceVideoProfile(int profile)
{
	return m_videoSourceSink->setVideoSourceVideoProfile((VIDEO_PROFILE_TYPE)profile, false);
}
